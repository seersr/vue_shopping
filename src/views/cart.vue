<template>
  <div>
              ...
              <li v-for="item in cartList" :key="item.productId">
                <div class="cart-tab-1">
                  <div class="cart-item-check">
                    <a href="javascipt:;" class="checkbox-btn item-check-btn" :class="item.checked ? 'checked':''" @click="changeProductNum('check',item)">
                ...
                <div class="cart-tab-2">
                  <div class="item-price">{{item.productPrice | priceFilter}}</div>
                </div>
                <div class="cart-tab-3">
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="select-self-area">
                        <a class="input-sub" @click="changeProductNum('minus',item)">-</a>
                        <span class="select-ipt">{{item.productNum}}</span>
                        <a class="input-add" @click="changeProductNum('add',item)">+</a>
                      </div>
                    </div>
                  </div>
                </div>
                <div class="cart-tab-4">
                  <div class="item-price-total">{{item.productNum * item.productPrice | priceFilter}}</div>
                </div>
                <div class="cart-tab-5">
                  <div class="cart-item-opration">
                    <a href="javascript:;" class="item-edit-btn" @click="showModel(item)">
          ...
          <div class="cart-foot-inner">
            <div class="cart-foot-l">
              <div class="item-all-check">
                <a href="javascipt:;" @click="toggleCheck">
                  <span class="checkbox-btn item-check-btn" :class="{'check':toggleCheckAll}">
                    <svg class="icon icon-ok">
                      <use xlink:href="#icon-ok" />
                    </svg>
                  </span>
            ...
            <div class="cart-foot-r">
              <div class="item-total">
                总价:
                <span class="total-price">{{totalPrice | priceFilter}}</span>
              </div>
              <div class="btn-wrap">
                <a class="btn btn--red" :class="{'btn--dis': !checkCounte}" @click="checkOut">结算</a>
              </div>
            </div>
          </div>
        </div>
    ...
    <vfooter></vfooter>
    <vmodel :showmodel='isshowModle' @closemodel="closeModel">
        <template v-slot:message>  
            <p>你确认要删除此条数据吗?</p>
        </template>
        <template v-slot:btnGroup>
              <button class="btn btn--m" href="javascript:;" @click="modelConfirm">确认</button>
              <button class="btn btn--m btn--red" href="javascript:;" @click="closeModel">关闭</button>
        </template>
    </vmodel>
  </div>
</template>
...
export default {
  ...
  methods:{
    //初始化购物车
    async init(){
      let res = await this.$axios.getCarList()
      this.cartList = res.data
      console.log(res)
    },
    //改变
    changeProductNum(type,item){
        if(type == 'minus'){
    ...
    //确定删除
    modelConfirm(){
      let tag = this.delCart
      this.cartList.forEach((item,index) => {
          if(tag.productId == item.productId){
