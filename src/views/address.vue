<template>
  <div>
      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
                <li v-for="(item,index) in showAddressList" :key="item.addressId" :class="{'check':checkedIndex==index}" @mouseenter="checkAddress(index)">
                  <dl>
                    <dt>{{item.userName}}</dt>
                  ...
                  <div class="addr-opration addr-del">
                    <!-- 删除地址 -->
                    <a href="javascript:;" class="addr-del-btn" @click="deleteAddress(index)">
                  ...
                  <div class="addr-opration addr-set-default">
                    <a href="javascript:;" class="addr-set-default-btn" v-show="!item.isDefault" @click="setDefaultAddress(index)">
                      <i>设为默认</i>
                    </a>
                  </div>
            ...
            <div class="shipping-addr-more">
              <a class="addr-more-btn up-down-btn" href="javascript:;" @click="showMore" :class="{'open':showAddressListCount!=3}">
                查看更多
                <i class="i-up-down">
    ...
    <vfooter></vfooter>
    <vmodel :showmodel="isshowmodel" @closemodel='isshowmodel = false'>
        <template v-slot:message>
              确定要删除吗?
        </template>
        <template v-slot:btnGroup>
              <button class="btn btn--m" href="javascript:;" @click="modelConfirm">确认</button>
              <button class="btn btn--m btn--red" href="javascript:;" @click="isshowmodel = false">关闭</button>
        </template>
    </vmodel>
  </div>
</template>
...
export default {
  ...
  computed:{
      showAddressList(){
        return this.addressList.slice(0,this.showAddressListCount)
      } 
  },
  methods:{
    async init(){
        let res =  await this.$axios.getAddress()
    ...
    //查看更多
    showMore(){
      if(this.showAddressListCount > 3){
          this.showAddressListCount = 3
      }else {
        this.showAddressListCount = this.addressList.length
      }
    ...
    deleteAddress(index){
      ...
      this.delAddressTem = index
    },
    modelConfirm(){
      this.addressList.splice(this.delAddressTem,1)
